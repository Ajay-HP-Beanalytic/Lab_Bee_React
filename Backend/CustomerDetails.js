...
function customerDetailsAPIs(app) {
  ...
  app.post("/api/addNewCompanyDetails", (req, res) => {
    ...
    const sqlQuery = `INSERT INTO customers_details (company_name, company_address, contact_person, customer_email, customer_contact_number, company_id, customer_referance) VALUES (?,?,?,?,?,?,?)`;
    ...
    db.query(
      ...
      (error, result) => {
        if (error) {
          ...
          res
            .status(200)
            .json({ message: "Company details added successfully" });
        }
      }
    );
  });
  ...
  app.get("/api/getCompanyDetails", (req, res) => {
    const sqlQuery = `SELECT id, company_name, company_address, contact_person, customer_email, customer_contact_number, company_id, customer_referance FROM customers_details`;
  ...
  app.post("/api/addNewCompanyDetails/:id", (req, res) => {
    ...
    const sqlQuery = `UPDATE customers_details SET company_name= '${companyName}', company_address= '${toCompanyAddress}', contact_person= '${kindAttention}', customer_email= '${customerEmail}', customer_contact_number= '${customerContactNumber}', company_id= '${customerId}', customer_referance= '${customerReferance}' WHERE id=${id}`;
    ...
    db.query(deleteQuery, [id], (error, result) => {
      if (error) {
        return res.status(500).json({
          error: "An error occurred while deleting the customer data",
        });
      }
  ...
  app.get("/api/getCompanyDetails/:companyId", (req, res) => {
    ...
    const sqlQuery = `SELECT company_name, company_address, contact_person, customer_email, customer_contact_number, company_id, customer_referance FROM customers_details WHERE company_id = ?`;
