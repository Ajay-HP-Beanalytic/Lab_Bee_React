...
function emiJobcardsAPIs(app, io, labbeeUsers) {
   ...
   *
   * @param {string} tableName - Table where change occurred (e.g., 'emi_jobcards', 'emi_tests_details_table')
  ...
  const logAuditTrail = async (
    ...
    try {
      ...
      const sql = `
        INSERT INTO emi_job_card_audit_trail (
          table_name,
          record_id,
          jc_number,
          field_name,
   ...
   */
  const updateParentEMIJobCardLastModified = async (jcNumber, updatedBy) => {
    try {
  ...
  // API to fetch audit trail for a specific Job Card
  app.get("/api/emi/jobcard/audittrail/:jc_number", async (req, res) => {
    const { jc_number } = req.params;

    if (!jc_number) {
      return res.status(400).json({
        success: false,
        message: "Job Card number is required",
      });
    }

    try {
      // Decode the JC number (it might contain slashes which are URL encoded)
      const decodedJcNumber = decodeURIComponent(jc_number);
      ...
      const sql = `
        SELECT 
          ...
          user_role,
          changed_at
        FROM emi_job_card_audit_trail
        WHERE jc_number = ?
        ORDER BY changed_at DESC
      `;

      const [rows] = await db.promise().query(sql, [decodedJcNumber]);
      ...
      return res.status(500).json({
        success: false,
        message: "Internal server error fetching audit history",
      });
    }
  });
  ...
  app.post("/api/EMIJobcard", async (req, res) => {
    ...
    try {
      ...
      if (eutTableRows && eutTableRows.length > 0) {
        ...
        jcNumber, eutName, eutQuantity, eutPartNumber, eutModelNumber, eutSerialNumber, eutUnitPowerRating, lastUpdatedBy)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  app.delete("/api/EMIJobcard/:id", async (req, res) => {
    ...
    try {
      ...
      // Delete tests
      await connection.query(`DELETE FROM emi_tests_table WHERE jcNumber = ?`, [
        jcNumber,
      ]);

      // Delete EUT data
      await connection.query(`DELETE FROM emi_eut_table WHERE jcNumber = ?`, [
        jcNumber,
      ]);
      ...
      for (let eutRow of eutTableRows) {
        if (eutRow.id) {
          ...
          await logAuditTrail(
            ...
            "CREATE",
            null,
            JSON.stringify(eutRow), // Store new row data
            loggedInUser,
            loggedInUserRole,
            loggedInUserDepartment,
          );
      ...
      for (let testPerformedRow of testPerformedTableRows) {
        ...
        if (testPerformedRow.id) {
          ...
          await logAuditTrail(
            ...
            "CREATE",
            null,
            JSON.stringify(testPerformedRow),
            loggedInUser,
            loggedInUserRole,
            loggedInUserDepartment,
          );
