...
const UpdateEnvironmentalQuote = () => {
    ...
    const handleSubmitAndUpdateETQuotation = async (e) => {
        ...
        /* const envitestQuotesTableDataRequest = axios.post(`http://localhost:4000/api/updateEnvitestQuotationData/` + quotationID, {
            tableData
        }); */

        try {
            // Wait for the both requests to finish
            /* const [quotesCategoryResponse, envitestTableResponse] = await Promise.all([quotesCategoryRequest]) */
            // const quotesCategoryResponse = await Promise.all(quotesCategoryRequest)

            // Check the status of both requests:
            /* if (quotesCategoryResponse.status === 200 && envitestTableResponse.status === 200) { */
            // if (quotesCategoryResponse.status === 200) {
            //     toast.success("Data Added Successfully")
            // } else {
            //     toast.error("An error occurred while saving the primary details.");
            // }
        } catch (error) {
            ...
            // } else {
            //     toast.error("An error occurred while saving the tests details.");
            // }
        }
 ...
 */
    ...
    const generatePdfFile = useReactToPrint({
        ...
        onAfterPrint: () => toast.success('Pdf file generated successfully')
    })
