...
const CreateTask = () => {
  ...
  //         // setValue("assigned_to_name", taskData.assigned_to_name);
  //         setValue("story_points", taskData.story_points);
  //         setValue("estimated_hours", taskData.estimated_hours);
  //         setValue("actual_hours", taskData.actual_hours);
  //         // setValue("task_assigned_date", dayjs(taskData.task_assigned_date));
  //         // setValue("task_due_date", dayjs(taskData.task_due_date));
  //         // setValue("task_completed_date", dayjs(taskData.task_completed_date));
  //         setValue(
  //           "task_assigned_date",
  ...
  //       fetchTaskDetailsAndPopulateForm(taskIdFromParams);
  //     } else {
  //       setIsEditMode(false);
  //       setSelectedTaskId(null);
  //       reset();
  //     }
  //   }, [taskIdFromParams, reset, setValue, navigate]);

  // Fetch team members first, then handle task data if needed
  useEffect(() => {
    const initializeData = async () => {
      // First fetch team members
      await fetchTeamMembersFromDatabase();

      // Then handle task data if in edit mode
      if (taskIdFromParams) {
        setIsEditMode(true);
        setSelectedTaskId(taskIdFromParams);
  ...
  // Additional useEffect to re-populate form when team members are loaded in edit mode
  useEffect(() => {
    if (
      isEditMode &&
      selectedTaskId &&
      ...
      fetchTaskDetailsAndPopulateForm(selectedTaskId);
    }
  }, [reliabilityMembers, softwareMembers, administrationMembers]);
