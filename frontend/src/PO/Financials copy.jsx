    ...
    <>
      ...
      {tabValue === 1 && (
        <Grid container spacing={3}>
              ...
              <Card>
                <CardContent>
                  ...
                  <ResponsiveContainer width="100%" height={400}>
                    <PieChart>
                      <Pie
                        data={dashboardData.departmentWiseData}
                        dataKey="totalInvoices"
                        nameKey="department"
                        cx="50%"
                        cy="50%"
                        ...
                        innerRadius={60}
                        paddingAngle={2}
                        label={({ department, totalInvoices, percent }) =>
                          `${department}: ${totalInvoices} (${(
                            percent * 100
                          ).toFixed(0)}%)`
                        }
                        labelLine={false}
                      >
                        {dashboardData.departmentWiseData.map(
                          (entry, index) => {
                            const colors = [
                              "#2196f3",
                              "#4caf50",
          ...
          {/* Optional: Dual-Axis Chart for Advanced Users */}
          <Grid item xs={12}>
            <Card>
              <CardContent>
                <Typography variant="h6" sx={{ color: "#003366", mb: 1 }}>
                  {/* Revenue vs Invoice Count (Dual Scale) */}
                  Revenue vs Invoice Count
                </Typography>
                <ResponsiveContainer width="100%" height={400}>
                  <ComposedChart data={dashboardData.departmentWiseData}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="department" />
